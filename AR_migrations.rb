...
# We also define how to undo the migration in down: we drop the table. We'll talk about why you might want to roll back a migration later.
...
# Writing a migration
# Inside your up/down or change methods, you may call methods of the parent class ActiveRecord::Migration. Here we go through some of the most common.
...
# change_column :table_name, :column_name, :type, options_hash; you can: 0. Change the data type of a column,
...
# Running migrations
# To actually perform the migrations you have written but not yet run, use the command rake db:migrate. This will look in the db/migrate directory, find all unexecuted migrations, and run their up or change methods in order of creation time. Only when you run the Rake task is the database actually modified.
...
# Occasionally you will make a mistake when writing a migration. If you have already run the migration then you cannot just edit the migration and run the migration again: Rails will think it has already run the migration and so will do nothing when you run rake db:migrate. That's because the timestamp is in schema_migrations.

# You must first rollback the migration, which reverses the change (by calling the down), if that is possible. This will undo the changes and remove the timestamp from schema_migrations.
# To rollback the most recent migration, run rake db:rollback. You may now edit the migration file and rerun.

# Don't edit old migrations
# Editing migrations often causes some trouble. It is a common mistake to begin editing the migration before rolling it back. Then, when you try to roll back, ActiveRecord tries to rollback the migration as it is currently written. This causes problems becauses the edited migration does not correspond to the migration that was actually previously run. You need to be careful of this: wait until after rollback to edit.

# It is okay to edit recent migrations as part of your development process. You'll make mistakes. But after you have written a migration, committed it, and pushed it to github, do not return to edit it. Treat that as a permanent part of the historical record of your schema's evolution.
...
# A few weeks later, you decide that you want to add a new column. If you tried to edit the previous migration, you would have to first reverse it. This would involve dropping the users table and all of its data. You could then edit and re-run, but your data will be gone.

# Even if you only push your code to development ("development" means the git branch that consists of recent code not yet deployed to the "production", user-facing application), you will frustrate other developers if you edit old migrations. Say I run your original users migration. You then edit it and push your code. When I fetch your code, running rake db:migrate will not run your changed migration. Instead, I have to myself reverse the old version of the migration (which I will have to recover; you checked in the edited version...), then re-run the new version.

# The answer is simple in these cases: you should write a new migration that performs the changes you require. Only edit existing migrations if they have never been pushed to git; you are only allowed to edit migrations that exist solely on your own development machine.
...
# You shouldn't worry too much about the schema.rb file for quite a while. Just do one thing: check in the changes that result from running rake db:migrate whenever you write a new migration. That will keep it from cluttering up your git status, and your repo will always have an up-to-date, authoritative description of the database schema.

# I should note that I have seldom used rake db:schema:load to load the schema.
...
# The solution is to add an index in an ActiveRecord migration. We'll use the add_index method:
